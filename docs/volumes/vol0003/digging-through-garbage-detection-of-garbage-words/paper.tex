...
%To train our models we created a labeled dataset.
...
\begin{table}[h!]
\caption{Character-based features used in the study}
\label{table:featurecreation}
\centering
\begin{tabular}{@{} p{3.5cm} p{6.5cm} p{2.8cm} p{4.2cm} @{}}
\toprule
 & \textbf{Feature} & \textbf{Calculation} & \textbf{Example for the word \texttt{Stroopwáfel}} \\
\midrule
\textbf{Adopted from\\Wudtke et al.} &
word length & \textit{L} & 11 \\
&
...
\midrule
\textbf{Based on\\Wudtke et al.} &
ratio of characters other than punctuation or numbers \textit{(o)} & \textit{o/L} & 0 \\
&
ratio of punctuation characters \textit{(p)} & \textit{p/L} & 0 \\
&
ratio of uppercase characters \textit{(u)} (except first letter) & \textit{u/L} & 0 \\
&
number of the maximum same consecutive characters & -- & 2 \\
\midrule
\textbf{New added features} &
ratio of both vowels and consonants & \textit{(v+c)/L} & 1 \\
&
...
&
number of same consecutive characters (diacritics stripped) & -- & 2 \\
&
number of consecutive vowels (diacritics stripped) & -- & 2 \\
&
number of consecutive consonants (diacritics stripped) & -- & 3 \\
\bottomrule
\end{tabular}
\end{table}
