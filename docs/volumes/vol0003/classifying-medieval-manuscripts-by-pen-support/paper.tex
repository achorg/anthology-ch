...
%\renewcommand{\hl}[1]{#1}\renewcommand{\todo}[1]{\relax}
...
%, \hl{curated from the SfarData, HebrewPal, and Ktiv collections},\todo{should we keep the names of sources here or check conclusion} 
...
%, thought to have originated in the ancient Pergamon, Anatolia region, 
...
%\todo{papyrus can also be written on both sides}  
...
% \begin{figure}[t]
%   \centering % lt bot rt top (in pts)
%     \includegraphics[trim=500 1450 900 255,clip,width=0.4\linewidth]{figures/calamus on parchment.jpg} % Leningrad codex
%     %\includegraphics[trim=100 200 100 55,clip,width=0.4\linewidth]{figures/quill on parchment.jpg}
%     \includegraphics[trim=620 580 300 457,clip,width=0.4\linewidth]{figures/MunichTalmud.png} 
% \\
%   \includegraphics[trim=345 350 205 419,clip,width=0.4\linewidth]{figures/maimonides.jpg} %% Commentary on Mishna calamus; (not quill) on paper; autograph?
% \includegraphics[trim=300 914 350 310,clip,width=0.4\linewidth]{figures/quill on paper.jpeg} % Sefer Hahinukh
...
%   The copies range in date from the early 11th century until the second half of the 15th; their provenance, from France to Italy to Egypt.}
  \label{fig:examples}
%   
% \end{figure}
...
%The partial English version of this work is hosted by the National Library of Israel.\footnote{\url{https://www.nli.org.il/en/discover/manuscripts/hebrew-codicology}}    
...
%\todo{don't start sentences with reference number} 
...
% For the substrate classification task (see \autoref{tab:substrate-data}), we curated a corpus of 1250 manuscript images, based on material annotations extracted from SfarData, and the images from Ktiv. 
% Black-and-white images were excluded to ensure consistency in visual characteristics. 
% Of these, 456 manuscripts were identified as paper and 794 as parchment. 
% To mitigate class imbalance, two images were sampled per paper manuscript and one per parchment manuscript, resulting in a dataset with balanced representation. 
% The final dataset was partitioned into training, validation, and test sets with no manuscript overlap to prevent data leakage: 874 manuscripts (about 70\%) for training (319 paper; 555 parchment), 188 for validation (69 paper; 119 parchment), and 188 for testing (68 paper; 120 parchment).
...
% The dataset used for the writing-tool classification task (see \autoref{tab:tool-data}) is based on data from HebrewPal. 
% The high-resolution color images were extracted from Ktiv. Following the recommendation in~\cite{jimaging}, we cropped nine non-overlapping patches from each image.
% The dataset comprises 80 manuscript images (40 per category), resulting in 720 extracted patches (360 per class). 
% Each patch was further analyzed to estimate the number of text lines it contains, with the extraction algorithm calibrated to capture approximately six lines per patch, yielding an estimated total of 4,320 text lines across the dataset. 
% To prevent data leakage and ensure robust evaluation, all patches originating from a given manuscript were kept together when splitting the dataset into training and test sets.
...
%\todo{this was the split: In total, out of 1,422 colored manuscripts:v113 are classified as “both” (i.e., both paper and parchment), 59 are classified as unknown, 456 are classified as paper, 794 are classified as parchment.}
...
%\todo{See Berat's comments in Overleaf. Give  thresholds in the text @Omer}(e.g.\@ blank margins) are discarded. 
...
%\todo{better to highlight than italic for new stuff to pay attention to}
...
%\todo{I moved tables to top of a close page. Only if they don't have a caption and are integral to the text should they be in the middle.}
...
%A summary of precision, recall, and $F_1$ score metrics, along with the confusion matrix, is provided in Table. 
...
%Precision, recall, F1-score and support per class; (b) Confusion matrix for support classification task.}
...
%It would be very helpful to understand these issues better.  
...
\begin{figure}[t]
    ...
    %\includegraphics[width=0.23\textwidth]{figures/gradcam_3.png}
    %\includegraphics[width=0.23\textwidth]{figures/gradcam_4.png}
    \includegraphics[width=0.23\textwidth]{figures/gradcam_5.png}
    \includegraphics[width=0.23\textwidth]{figures/gradcam_6.png}
    %\includegraphics[width=0.23\textwidth]{figures/gradcam_7.png}
...
%Future work will expand this framework to include further substrates and refine classification models to capture subtle textural information. 
...
%\todo{never use "and others" in the bib file}
\printbibliography
