...
\caption{Visualising how models differ in their similarity assessment (\textit{ft\_best} vs. \textit{base}). Black dots indicate disagreement.\label{fig:diff_ft_best}}
\end{figure}
...
\end{tabular}
\caption{Zero-shot classification results.\label{table:zs}}
\end{center}
...
Names seem somewhat easier to classify compared to the more abstract and generic nouns: the scores climb and the fine-tuned models outperform SigLIP \textit{base} by a margin of more than 10\%.
...
\bottomrule
\end{tabular}
\caption{Average similarity (for image-to-image search) between names of the query record and retrieved records. The first set of results is collection agnostic; the second set shows results for cross-collection search.\label{table:searchimgtoimg}}
\end{center}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Average similarity between the name of the query record and retrieved records for image-to-text and text-to-image search.\label{table:searchother}}
\end{center}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Logistic regression that predict the effect of the model selection on link annotation.\label{table:reg1}}
\end{center}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Logistic Regression that estimates the effect of modality and model on link annotation.\label{table:reg2}}
\end{center}
\end{table}
