...
%Conjoint analysis and vignette factorial surveys have been widely used to analyze human preferences in many areas of social science \cite{hainmueller_causal_2014, wallander_25_2009} and have been proven to replicate similar trends to the ones traced by observational analyses in real-world decisions \cite{hainmueller_validating_2015}. Furthermore, the data resulting from this survey design can be used to identify causal effects.
...
\section{Results}
%We employ a survey experiment in which participants read two stories, one with a woman character (using she/her pronouns) and one with a man character (using he/him pronouns). One story is about a hike, the other about a coffee shop (full stories are available in Appendix~\ref{appdx:A}). Participants read the stories in random order, and we randomize whether a woman character is featured in the hike or the coffee shop story, with a man character featured in the other story (both cases identified solely by gender-indicating pronouns). Participants are then asked to choose which story they would continue reading. Using the crowd-sourcing platform Prolific, we recruit 3,000 participants. After excluding participants who do not self-identify as women or men, we obtain a total of 2,983 responses ($W = \text{1,492}, M = \text{1,491}$). More details on the recruitment process and survey design are available in the \emph{Materials and Methods} section below.
...
\begin{figure}[htb]
  ...
  \label{fig:hikeprob}
% \alt{A bar plot with error bars showing the probability of choosing the hike story within treatment conditions and subgroups.}
...
\begin{figure}[htb]
  ...
  \label{fig:cate}
% \alt{A point plot with error bars showing the average treatment effect within each subgroup.}
