...
%This has been important for all kinds of databases in various fields and thus it has also been known under many names. It can be called entity resolution\cite{christophides2020overview}, record linkage\cite{winkler2014matching}, merge-purge\cite{hernandez1995merge}, duplicate detection\cite{papenbrock2014progressive}, deduplication\cite{xia2016comprehensive}, reference reconciliation\cite{sais2007l2r} and many other terms. These names all have their own specific application and meaning, but the essence remains the same (some researchers speculate the term itself could use entity matching \cite{barlaug2021neural}). What the terms have in common is the task of grouping of individual records to combined entities. 
...
%The disadvantages of rule-based or deterministic systems is their rigidity, they can't deal with the ambiguity of rare irregularities, thus variable forms of data entry still form an issue. Machine learning methods were introduced to deal with this problem, and tend to deal better with the distributed nature of the irregularities in most real-world datasets \cite{kopcke2010frameworks}.

%Classifiers like support vector machines could be trained with features like TF-IDF vectors, or more carefully hand-selected features.
%A regular entity matching method will be quadratic in complexity ($O_{n^2}$), since each record needs to be compared to each other record to find a match. That's why blocking and filtering techniques have become an integral part of Entity Matching\cite{papadakis2020blocking}.
...
%These results are mixed however comparing different domains. Interestingly, in one experiment for the DBLP-scholar database which containing metadata from a bibliographic scholarly database, the pre-trained model Ditto had better performance than any of the LLM's\cite{peeters2023entity}.

%Much has been written about the popular FRBR framework, but actual implementation has been rare\cite{o2018frbr}. One of the reasons for that is the lack of a standardized definition of entities.
%
%An area of discussion around the FRBR model is the distinction between work and expression entities. The OCLC work-set algorithm has specifically omitted this distinction because they consider there is too little information in the metadata to identify similar or different expressions\cite{hickey2002experiments}. Another popular data model Bibframe, also forgoes the distinction\cite{kroeger2013road}. However, according to user studies, having separate work and expression entities is seen as useful\cite{pisanski2012user}.
...
%\item \label{exhulst} COL taal VAL ned COL jaar VAL 1918 COL titel VAL @Van een klein meisje en een groote klok COL hoofdauteur vermelding VAL door W.G. van de Hulst COL 2e auteur vermelding VAL [illustraties van Frans van Naarden en O. Geerling] COL hoofdauteur VAL Hulst, Willem Gerrit van de Hulst 1879-1963 COL 2e auteur VAL Naarden, Frans van Naarden COL 2e auteur VAL Geerling, Otto Geerling 1850-1930 COL editie VAL 3e dr COL plaats VAL Nijkerk COL uitgever VAL G.F. Callenbach COL omschrijving VAL Bekroond door de Nederlandsche Zondagsschoolvereeniging

%COL taal VAL ned COL jaar VAL 1974 COL titel VAL @Van een klein meisje en een grote klok COL hoofdauteur vermelding VAL W.G. van de Hulst COL 2e auteur vermelding VAL tekeningen van Sierk Schröder COL hoofdauteur VAL Hulst, Willem Gerrit van de Hulst 1879-1963 COL 2e auteur VAL Schröder, Sierk Carl Schröder 1903-2002 COL editie VAL 14e dr COL plaats VAL Nijkerk COL uitgever VAL G.F. Callenbach COL omschrijving VAL Bekroond door de Nederlandse Zondagsschoolvereeniging  
\end{examples}
...
%A big question left open is what to do with aggregates. These are bundles, anthologies, omnibuses containing multiple works under single records. We do not know how the models would perform on these and how LLMs could best disentangle them. Handling aggregates is also a bigger issue in discussions around FRBR.

%Another potential avenue for further research is further metadata inclusion or exclusion. We only tested one variation, a minimal case, and a maximal case. What happens when removing or adding metadata fields one by one? 

%We also need to consider end-to-end performance. We evaluated performance on candidate pairs and left out blocking steps which are necessary in a real-world application where millions of books need to be compared. Entity matching models like Ditto measure performance while including a blocking step\cite{li2020deep}, we would also need to name these entities for a full end-to-end implementation. Additionally, LLMs lack explainability, the KB wants to include a human-in-the-loop so for an end-to-end application, this is a serious drawback. 
