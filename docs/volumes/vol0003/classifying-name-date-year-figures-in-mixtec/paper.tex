...
\author[1]{Christopher Driggers-Ellis}[ % Christopher
  orcid=0009-0006-8063-7571
]

\author[1]{Christan Grant}[ % Dr. Grant
...
\end{itemize}
%This work demonstrates the potential of transformer-based vision models in the decoding of semasiographic scripts and contributes to the broader goal of applying AI to the preservation of cultural heritage.
...
%The ViT-B/16 architecture, introduced by, divides each input image into non-overlapping 16$\times$16 patches, which are then linearly embedded and fed into a standard Transformer encoder. 
...
We perform fine-tuning using a single A100 GPU, with a batch size of 32 and up to 16 parallel data-loading workers. We trained each classifier for 10 epochs, as visual inspection of the loss curves in \Cref{fig:training-loss} indicated plateaus in training and test loss.%, with training loss largely plateaued and test loss showing signs of stabilization. 
...
%We freeze all the layers except the last layer of the model to retain general visual features and fine-tune only the final layer and classification head for each of the three classifiers.
...
%In particular, our classifiers address the calendrical dimension of Mixtec narratives, which is critical for determining when an event occurred and who was involved. This complements visual scene interpretation pipelines and offers a clear path toward a full machine-based interpretation of semasiographic manuscripts.
...
% \section{Model Training Curves} \label{appdx:first}

% % --- Figures for the appendix, placed directly after the section heading ---
% % Removed float specifiers to try and force "here" placement.
% % If they still float, the package 'float' with [H] would be the next step,
% % but template guidelines suggest avoiding extra packages.

% \begin{figure}[h] % No float specifier to force placement
%     \centering
%     \includegraphics[width=0.5\linewidth]{namedate-year (2).png}
%     \caption{Accuracy and loss curves for the \textbf{Name-Date/Year Type Classifier}. The model converges well and shows minimal overfitting.}
  \label{fig:type-classifier}
%     
% \end{figure}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.5\linewidth]{symbol (1).png}
%     \caption{Performance curves for the \textbf{Symbol Classifier}. The model generalizes well across glyph types.}
  \label{fig:symbol-classifier}
%     
% \end{figure}

% \begin{figure}[h] 
%     \centering
%     \includegraphics[width=0.5\linewidth]{year (1).png}
%     \caption{Training and test curves for the \textbf{Bead Counter}. The model struggles to generalize, reflecting limitations in counting tasks.}
  \label{fig:bead-counter}
%     
% \end{figure}
\end{document}
