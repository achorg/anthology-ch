...
\paragraph{Sampling the solution landscape.} Set sample size \(n\)
...
\paragraph{Compute edge co-membership score.} This score  \(c_{ij}\) in
...
\paragraph{Compute edge-level ambiguity.} We use a straightforward
...
\paragraph{Compute node-level ambiguity.} We simply aggregate the
...
\paragraph{Perceptive rationale.} For simplicity, let us assume a sharp
distinction between bridging edges, in-cluster edges, and ambiguous
edges (Figure~\ref{fig:figure1}). In reality, the distinction is continuous and determined
...
well-defined clusters (Figure~\ref{fig:figure2}), community detection is typically stable
and few nodes will be ambiguous (Figure~\ref{fig:figure2} has few white nodes and edges).
...
When the network does not have well-defined clusters (Figure~\ref{fig:figure3}), ambiguous
nodes and edges are much more common (Figure~\ref{fig:figure3} has more white than
Figure~\ref{fig:figure2}). Nevertheless, the ambiguity is unevenly distributed and often
concentrated in the middle of the network, while some areas, often on
the sides, remain relatively stable (colored subclusters in Figure~\ref{fig:figure3}).
...
The effect can be seen more clearly in Figure~\ref{fig:figure4}, representing a finite
square lattice, where communities are consistently found in each corner
(color in Figure~\ref{fig:figure4}) while the middle and sides are ambiguous (white in
