...
\small
\centering
\begin{tabular}{p{0.23\linewidth} r r r | r r r r r}
\toprule
...
We then performed unsupervised clustering with the K-means algorithm as implemented in \texttt{scikit-learn v.1.4.2}.
...
Thus, the dramatic \textit{Angst} genre appears associated with negativity mainly in a relative sense, having the largest subset of tragic structures. The steepest and most frequent positive inclines characterize the \textit{Hurt/comfort} genre, followed by \textit{Fluff}. Meanwhile, on average, \textit{Fluff} has the most positive peaks.
