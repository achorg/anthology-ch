...
\begin{itemize}
\item \textbf{Example mention}: \emph{...γὰρ ἡ Κύπρις πικρά [Ms] Ἀχαιός [Me] φησιν ἐν Αἴθωνι σατυρικῷ. παρ’ οὗ ὁ σοφὸς Εὐριπίδης λαβὼν ἔφη...} \\
...
\newpage
\subsection{Silver data} \label{sec:silver_data}
...
The approach made it possible to go from the entry of the \textit{RE} discussing the Ancient Greek tragedian Achaios (Achaios 6), who, based on the \textit{RE Volltext} entry, is mentioned in the sixth book, section 270C, of Athenaeus \textit{Deipnosophistae}, to the corresponding passage in the GLAUx corpus; we could then annotate the token Ἀχαιός (Achaiòs)  as a mention of Achaios 6 of the \textit{RE}.\footnote{We excluded the cases in which the specific passage only gave information about the book in which the person was mentioned, because it is a very broad unit that could potentially introduce errors in the pipeline.}
...
\midrule
\multirow{2}{*}{\bf gold} & Kurz (10 epochs) & 72 \% & 37 \% & 26 \% \\ 
                          & Voll (10 epochs) & 61 \% & 40 \% & 24 \% \\ 
\multirow{2}{*}{\bf gold + silver} & Kurz (5 epochs) & 76 \% & 37 \% & 28 \% \\ 
                   & Voll (10 epochs) & 69 \% & 42 \% & 29 \% \\
                   \midrule
...
The results are significantly worse those than benchmarked performance on unseen mentions for English data (76.8 \% total accuracy, as reported in \textcite{Wu_Petroni_Josifoski_Riedel_Zettlemoyer_2020}). Some performance drops are expected given the lower-data scenario and the multilingual setting. However, even in the best performing setting the model reaches only 29\% overall accuracy. Adding the lower-quality silver data does improve performance considerably, especially bi-encoder recall.
Beside this, the  \textit{Volltext}-where-available model slightly underperforms compared to the \textit{Kurztext}-only model in terms of bi-encoder recall, but makes up for this difference in cross-encoder accuracy, leading to an overall marginally superior performance.The results are further discussed in Section \ref{sec:analysis}.
...
\hfill
\begin{minipage}[b]{0.4\textwidth}
\includegraphics[width=\textwidth]{figures/voll_accuracy_threshold.png}
...
οἱ μὲν οὖν Ἀχαιοὶ συνεληλυθότες εἰς Αἴγιον ἐκεῖ τὸν Ἄρατον ἐκάλουν., (Accordingly, the Achaeans came together at Aegium and invited Aratus thither, Plutarch \textit{Aratus}, 42.1, Transl. Bernadotte Perrin) the model may struggle to distinguish between these two entities, because this context does not provide sufficiently distinctive features. This suggests that richer forms of context, such as temporal metadata, may be necessary to resolve these ambiguities reliably.
...
& & rc@64 & cross acc & overall acc  \\
\midrule
\multirow{2}{*}{\bf gold} & Kurz (10 epochs)  & 65 \% &  28 \% & 18 \% \\ 
& Voll (10 epochs) & 61 \% & 29 \% &  18\% \\ 
\multirow{2}{*}{\bf gold + silver} & Kurz (5 epochs)  &  77 \% & 37 \% & 29\% \\ 
& Voll (10 epochs) & 75\% & 44\% & 33\% \\
\midrule
