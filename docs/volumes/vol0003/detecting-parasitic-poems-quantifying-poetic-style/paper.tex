...
% \usepackage[backend=biber,style=authoryear,maxnames=99,maxcitenames=99]{biblatex}
% or if you prefer numeric style:
% \usepackage[backend=biber,style=numeric,maxnames=99,maxcitenames=99]{biblatex}
...
\begin{table}[h]
  \centering 
  \begin{tabular}{ccc p{3cm}}  % changed the last column to p{}
    \toprule
...
\begin{itemize}
    \item \textbf{Prompt 3:} \textzh{请判断下面诗句相对于其上下文是否是“装饰性”使用。"装饰性"指}: Current definition for ``decorative poems" is that the poem does not add much meaningful information to its immediate contexts (e.g., repeating what has already been said in the contexts).\\
    \textzh{只回答“是”或“否”。}\\
    \textzh{上文:} \{before\} \\
    \textzh{诗句:} \{poem\}\\
    \textzh{下文:} \{after\}\\
    \textzh{如果有请直接回答1，如果没有请直接回答0，不需要解释。}
\end{itemize}
