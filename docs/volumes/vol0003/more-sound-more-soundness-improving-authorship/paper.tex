...
%Arthur Rimbaud, born in 1854, and Germain Nouveau, born in 1851, are almost the same age: they met in 1873 in Paris and were sufficiently sympathetic to leave for London together the following year, approximately at the time of the writing of \textit{Illuminations} (the exact dating of which remains uncertain, see, for example, \cite{chadwick_date_1958}). On his return from England, we know that Rimbaud went to Stuttgart in February 1875 to meet Verlaine, recently released from prison after the ``Brussels tragedy'', and gave his former lover a manuscript that was said to be that of \textit{Illuminations}, asking that he pass it on to Germain Nouveau. This could have been done, if we are to believe a letter dated May 1, 1875, to Ernest Delahaye, in which Verlaine says he sent a package (the contents of which are not specified) to the author of \textit{Valentines}\footnote{\textit{Importants livres anciens, livres d'artistes et manuscrits}, Christie's sale of November 29, 2011, Paris, lot 138. \url{https://www.christies.com/en/lot/lot-5509554}.}. If a manuscript was sent, it was not Rimbaud's original, however; Verlaine lent it to Charles de Sivry, who would not return it to its owner until 1885, finally allowing publication in 1886, while Rimbaud was on an adventure in Africa.
...
%Concerning the phonic dimension of the language, linguists have thoroughly studied the complexity of the (ortho)graphic practice and its relation with speech or orality in synchrony \cite{stark_oral_2015} and diachrony \cite{parussa_enregistrer_2017}, but without a literary perspective. Reversely, in literature, the bulk of production is mainly concerned with the stylistic rendering of orality \cite{mahrer_phonographie_2017}, but not with the phonic value of the text in general. In general, stylistics or textometry \cite{pincemin_textometrie_2020} are more concerned by words and their context.
%Stylistics, on the other hand, is traditionally interested in sound effects (alliterations, assonance, paronomasia, homeoteleuton, etc.) or visual effects (rhyme for the eye, etc.), but only in a short textual space (a few sentences, sometimes a bit more). Only the aforementioned stylometric study on Rimbaud has approached the literary value of potentially sublexical units from a phonic point of view
...
%For instance, the nasals are represented with a diacritic tilde, similar to IPA notation.
...
\noindent
\textbf{Phonetic transcription.} This grapheme-to-phoneme task is applied independently to each token to generate a phonetic transcription. The following protocol is followed while performing this task:
\begin{enumerate}
...
%This is a clear limitation of this early work, but since the same processing was performed for all texts, the comparisons are still valid.
