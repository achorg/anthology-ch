...
%   \begin{tabular}{lrr}
%     \toprule
%     \textbf{Dimension} & \textbf{Count} & \textbf{Coverage}\\
%     \midrule
%     \multicolumn{3}{l}{\textit{Analytical Sample}} \\
%     \quad Articles with geocoded locations & 347,291 & 74.7\% of initial sample \\
%     \quad Total geocoded locations & 1,388,720 & 83.8\% of extracted \\
%     \quad Unique locations & 158,288 & -- \\
%     \multicolumn{3}{l}{\textit{Spatial Coverage}} \\
%     \quad News outlets & 360 & 100\% \\
%     \quad Local Authority Districts & 337 & 100\% \\
%     \quad Publishers & 87 & 100\% \\
%     \bottomrule
%   \end{tabular}
%   \caption{Final analytical sample characteristics.}
  \label{tab:final_sample}
%   
% \end{table}
...
\caption{Descriptive Statistics of Features}\label{tab:descriptives} 
\end{table}
...
\caption{Media Outlet Spatial Reach Typologies}\label{tab:cluster_profiles_expanded}
\end{table}
...
% \textbf{Radius}: We identified the primary location $\mathbf{x}_0 = \arg\max_{\mathbf{x}} n(\mathbf{x})$ (location with maximum mentions $n$), then computed distances $d_i = \|\mathbf{x}_i - \mathbf{x}_0\|$ for all locations. The radius $r_{75}$ is defined as:
% \[
% r_{75} = \min\left\{ r \;\Big|\; \sum_{d_i \leq r} n(\mathbf{x}_i) \geq 0.75 \cdot \sum_{\mathbf{x}} n(\mathbf{x}) \right\}
% \]
% This calculates the smallest radius from the primary location that contains 75\% of all location mentions by frequency. Distances are calculated via the Haversine formula.

% \textbf{Area}: For locations within $r_{75}$, we computed the minimum convex polygon $\mathcal{H}$ enclosing all points using the Jarvis march algorithm (via \texttt{concaveman}), then calculated its area in km\textsuperscript{2}:
% \[
% A = \iint_{\mathcal{H}} d\mathbf{x}
% \]
% This calculates the area of the smallest convex polygon that encloses all locations within the 75\% radius.

% \subsubsection{Administrative Reach}
% \textbf{District count}: The total unique districts $D$ covered by an outlet's mentions:
% \[
% D = \left|\left\{ \text{district}(x_i) \;|\; i = 1,\dots,N \right\}\right|
% \]
% This calculates the number of unique administrative districts mentioned across all location references.

% \textbf{Gini coefficient}: For demographic concentration across Supergroups $S_1,\dots,S_K$, with mention counts $n(S_k)$:
% \[
% G = \frac{\sum_{k=1}^K \sum_{l=1}^K |n(S_k) - n(S_l)|}{2K \sum_{k=1}^K n(S_k)}
% \]
% This calculates how unequally location mentions are distributed across different demographic area types (0 = perfectly equal distribution, 1 = perfectly concentrated). Location mentions were first assigned to the nearest administrative district via nearest-neighbour spatial joins, then classified using ONS Supergroup demographic categories (implemented via \texttt{ineq::Gini}).

% \subsubsection{Spatial Heterogeneity}

% \textbf{Shannon entropy}: For district-level mention proportions $p_i = n(d_i)/\sum n(d_i)$:
% \[
% H = -\sum_{i=1}^D p_i \cdot \log(p_i + \epsilon), \quad \epsilon = 10^{-10}
% \]
% This calculates how evenly location mentions are spread across districts, where higher values indicate more even coverage. The parameter $\epsilon$ provides numerical stability when $p_i = 0$. Low entropy ($H \approx 0$) indicates concentrated coverage, whilst high entropy (approaching $\log(D)$) reflects uniform geographic attention.

% \textbf{Moran's I}: For spatial autocorrelation of mentions at locations $\mathbf{x}_i$:
% \[
% I = \frac{N}{\sum_i \sum_j w_{ij}} \cdot \frac{\sum_i \sum_j w_{ij} (y_i - \bar{y})(y_j - \bar{y})}{\sum_i (y_i - \bar{y})^2}
% \]
% This calculates whether nearby locations have similar mention frequencies (positive values = clustering, negative values = dispersion, zero = random distribution). Here, $w_{ij} = \mathbb{I}(d_{ij} \leq 50\text{km})$ are binary spatial weights, $y_i = n(\mathbf{x}_i)$, and $N$ is total mentions. We selected the 50 km threshold following sensitivity analysis comparing 25, 50, and 75 km thresholds. Results showed stable outlet rankings between 50 km and 75 km thresholds (Spearman's $\rho = 0.95$), but greater divergence between 25 km and 75 km ($\rho = 0.75$), indicating that threshold choice meaningfully affects clustering detection for highly local outlets. The 50 km threshold balances local clustering capture with cross-outlet comparability.

% \subsubsection{Distance Decay}
% For each mention, distance to the outlet's primary location was calculated using the Haversine formula. Dispersion was summarised using:

% \textbf{Coefficient of variation}:
% \[
% CV = \frac{\sigma_d}{\mu_d}, \quad \mu_d = \frac{1}{N}\sum_i d_i, \;\; \sigma_d^2 = \frac{1}{N}\sum_i (d_i - \mu_d)^2
% \]
% This calculates how variable the distances are relative to the average distance, where higher values indicate more scattered coverage patterns.

% \textbf{Proportion within 10 km}:
% \[
% P_{10} = \frac{|\{d_i \leq 10\text{km}\}|}{N}
% \]
% This calculates the fraction of all location mentions that fall within 10 km of the outlet's primary location.
...
\caption{Clustering Method Performance Comparison}\label{tab:clustering_experiments}
\end{table}
