...
% xcolor with the [table] option is crucial for \columncolor
\usepackage[table]{xcolor}
% The 'array' package is needed for the >{} column specifier syntax
\usepackage{array}
...
% --- Column Definitions ---
% The >{\columncolor{...}} syntax applies a background color to the entire column.
% This is the key change to color all rows, not just the header.
\begin{tabular}{
...
% --- Table Header ---
% \cellcolor is no longer needed as the column color is now applied automatically.
\textbf{Love} &
\textbf{Joy} &
...
% --- Table Body ---
% The content remains the same. The column colors from the definition above
% will be applied automatically to every cell.
...
%   \caption{Emotions and the amount of annotated sections.}
  \label{tab:consensus_amotion_annos_ger}
%   
% \end{table}
