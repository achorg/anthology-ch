...
\usepackage{adjustbox} % multiple tables on a line
...
\end{array}
\right.
% \text{where}~\lambda_{roman}={1\over{max_{w \in \C}(d(f_{roman},f_{new},w))}}
\label{eq:sigma}
\end{equation}
...
% \sigma(w) = \left\{
% \begin{array}{r l}
% \lambda_0\sqrt[4]{\sqrt[4]{f_0(w)} -\sqrt[4]{f_1(w)}} & \text{if}~f_0(w) \geq f_1(w) \\
% -\lambda_1\sqrt[4]{\sqrt[4]{f_1(w)} -\sqrt[4]{f_0(w)}} & \text{otherwise.}
% \end{array}
% \right.
% \label{eq:sigma}
% \end{equation}
...
%From \emph{word specificity} $\sigma$ and tokenisation of the sentences, the equation for \emph{sentence temporal valence} $\rho$ is:
...
% \caption{Comparison overview of results for $\beta$.}
...
% \includegraphics[width=\linewidth,trim={-9 780 -8 10},clip]{077-fig-delta-stats.pdf}
...
% left bottom right top
% \quad 
 % \includegraphics[width=.5\textwidth,trim={20 0 0 0},clip]{079-fig-significance-new-heatmap.pdf}
% 
\end{adjustbox}
...
%\todo Note that the sign of Cohen's d does not always correspond with the direction of significance. This is due to the skew of the results per model (i.e. the upper tail in $\beta$ is larger than the lower one), but has no impact on the discussion below.
...
%Training multilingual models might be further complicated in the historical context where there is even further orthographic variation, and alphabets in particular.
