...
%The LLM showed robust performance even without detailed category definitions, suggesting it can extrapolate context from minimal instruction. Notably, it handled noisy OCR data effectively, likely thus outperforming more brittle systems more reliant on correct input. It also seems that it may even have distinguished metaphorical from literal usage in terms such as \textit{medicina} (which frequently appear as metaphors in, for instance,  religious texts rather than indicating a medical context), likely because of the presence of more concrete cues within the `anatomical' category.
...
%More frequently, the models sometimes defaulted to broader categories, such as `metaphorical general,' where a human annotator might select a more specific label like `spiritual/religious.' Less frequent or less clearly defined categories, such as `historical/political', were particularly prone to underuse or misclassification, possibly due to their rarity in training data or insufficient semantic clarity in how to distinguish it from other categories. 
...
%The primary contribution of our computational approach is not interpretive analysis, but rather a refined method of filtering historical texts, akin to an advanced form of keyword search. 
...
\noindent
\textbf{The aggregate corpus and its components} 
...
%TODO question for VK: but noscemus actually includes up to 19th century, is this not included in GreLa? 
