...
\noindent As the internal HTR team, we first select a diverse batch of images with different forms of Tibetan \textit{ume} scripts (see sample in Figure \ref{fig:corpus}). We then divide this into two separate batches: a `manual input' batch and a `correction' batch. Both batches are uploaded and backed up on the BDRC servers, and are also uploaded to a custom-made tool created by Esukhia for inputting and correcting transcriptions (shown in Figure \ref{fig:manual-pechatools}).
