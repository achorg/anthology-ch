...
def create_crossref_xml(volume: str, papers: List, verbose: bool = False) -> None:
    ...
    ET.SubElement(
        journal_metadata, "full_title"
    ).text = "Anthology of Computers and the Humanities"
    ...
    pub_date_issue = ET.SubElement(
        journal_issue, "publication_date", {"media_type": "print"}
    )
    ...
    for paper in papers:
        ...
        # Publication date
        pub_date_article = ET.SubElement(
        ...
        ET.SubElement(pages, "first_page").text = str(
            pmeta.get("publication_info", {}).get("pagestart", "1")
        )
        ET.SubElement(pages, "last_page").text = str(
            pmeta.get("publication_info", {}).get("pageend", "1")
        )
